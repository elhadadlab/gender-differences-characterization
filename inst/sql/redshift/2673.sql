...
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (436903,4102297,443746,434860,435785,442336,438418,433403)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (436903,4102297,443746,434860,442336,438418)
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= DATEADD(day,CAST(0 as bigint),P.END_DATE) AND A.END_DATE >= DATEADD(day,CAST(0 as bigint),P.START_DATE) AND A.END_DATE <= P.OP_END_DATE ) cc
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= DATEADD(day,CAST(0 as bigint),P.START_DATE) AND A.END_DATE >= DATEADD(day,CAST(0 as bigint),P.START_DATE) AND A.END_DATE <= P.OP_END_DATE ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
