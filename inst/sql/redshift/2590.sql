...
SELECT
event_id,  person_id ,
  case when DATEADD(day,CAST(30 as bigint),start_date) > op_end_date then op_end_date else DATEADD(day,CAST(30 as bigint),start_date) end as end_date
...
(
	SELECT
		person_id
		, DATEADD(day,CAST(-1 * 30 as bigint),event_date)  as end_date
	FROM
	(
		SELECT
			person_id
		...
		(
			...
			SELECT
				person_id
				, DATEADD(day,CAST(30 as bigint),end_date) as end_date
				, 1 AS event_type
				, NULL
			FROM #cohort_rows
		) RAWDATA
