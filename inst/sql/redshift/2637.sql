...
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4137430,441264,40321716,432870,4247776,440372)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4137430,441264)
  and c.invalid_reason is null
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,CAST(30 as bigint),P.START_DATE) AND A.START_DATE <= DATEADD(day,CAST(365 as bigint),P.START_DATE) ) cc
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= DATEADD(day,CAST(0 as bigint),P.START_DATE) AND A.END_DATE >= DATEADD(day,CAST(0 as bigint),P.START_DATE) AND A.END_DATE <= P.OP_END_DATE ) cc
GROUP BY cc.person_id, cc.event_id
HAVING COUNT(cc.event_id) >= 1
-- End Correlated Criteria
...
(
	SELECT
		person_id
		, DATEADD(day,CAST(-1 * 365 as bigint),event_date)  as end_date
	FROM
	(
		SELECT
			person_id
		...
		(
			...
			SELECT
				person_id
				, DATEADD(day,CAST(365 as bigint),end_date) as end_date
				, 1 AS event_type
				, NULL
			FROM #cohort_rows
		) RAWDATA
