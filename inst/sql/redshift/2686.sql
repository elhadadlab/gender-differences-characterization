...
select C.person_id, C.procedure_occurrence_id as event_id, C.procedure_date as start_date, DATEADD(d,CAST(1 as bigint),C.procedure_date) as END_DATE,
       C.visit_occurrence_id, C.procedure_date as sort_date, C.procedure_concept_id as domain_concept_id
from
(
  select po.*
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,CAST(-7 as bigint),P.START_DATE) AND A.START_DATE <= DATEADD(day,CAST(7 as bigint),P.START_DATE) ) cc
...
FROM #qualified_events E
JOIN @cdm_database_schema.PERSON P ON P.PERSON_ID = E.PERSON_ID
WHERE (EXTRACT(YEAR FROM E.start_date) - P.year_of_birth >= 50 and EXTRACT(YEAR FROM E.start_date) - P.year_of_birth <= 89)
GROUP BY e.person_id, e.event_id
-- End Demographic Criteria
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,CAST(-365 as bigint),P.START_DATE) AND A.START_DATE <= DATEADD(day,CAST(-8 as bigint),P.START_DATE) ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,CAST(-365 as bigint),P.START_DATE) AND A.START_DATE <= DATEADD(day,CAST(-1 as bigint),P.START_DATE) ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
GROUP BY p.person_id, p.event_id
HAVING COUNT(cc.event_id) = 0
-- End Correlated Criteria
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,CAST(0 as bigint),P.START_DATE) AND A.START_DATE <= DATEADD(day,CAST(0 as bigint),P.START_DATE) ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
GROUP BY p.person_id, p.event_id
HAVING COUNT(cc.event_id) = 0
-- End Correlated Criteria
