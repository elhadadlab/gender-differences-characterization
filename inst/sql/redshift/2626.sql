...
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (374021,43530701,443802,4190307,378143)
...
(
	SELECT
		person_id
		, DATEADD(day,CAST(-1 * 90 as bigint),event_date)  as end_date
	FROM
	(
		SELECT
			person_id
		...
		(
			...
			SELECT
				person_id
				, DATEADD(day,CAST(90 as bigint),end_date) as end_date
				, 1 AS event_type
				, NULL
			FROM #cohort_rows
		) RAWDATA
