...
WHERE (C.condition_start_date >= TO_DATE(TO_CHAR(2010,'0000FM')||'-'||TO_CHAR(1,'00FM')||'-'||TO_CHAR(1,'00FM'), 'YYYY-MM-DD') and C.condition_start_date <= TO_DATE(TO_CHAR(2020,'0000FM')||'-'||TO_CHAR(1,'00FM')||'-'||TO_CHAR(1,'00FM'), 'YYYY-MM-DD'))
...
SELECT
event_id,  person_id ,
  case when DATEADD(day,CAST(7 as bigint),start_date) > op_end_date then op_end_date else DATEADD(day,CAST(7 as bigint),start_date) end as end_date
...
(
	SELECT
		person_id
		, DATEADD(day,CAST(-1 * 180 as bigint),event_date)  as end_date
	FROM
	(
		SELECT
			person_id
		...
		(
			...
			SELECT
				person_id
				, DATEADD(day,CAST(180 as bigint),end_date) as end_date
				, 1 AS event_type
				, NULL
			FROM #cohort_rows
		) RAWDATA
