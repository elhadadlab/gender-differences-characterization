...
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (376680,40480055,374924,43530734,381842,373748,374915,4165417,4309257,40483317,43530626,37109663,4306496,374023,4311977,4029498,380378,377091,444413,4206148,80205)

) I
) C UNION ALL
SELECT 1 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (740275,798874,750119,21604429,745466,19050832,44507780,795661,797399,19087394,705103,711584,718122,42904177,734354,19006586,715458,742267,19020002,744798,740910,734275,751347)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (740275,798874,750119,21604429,745466,19050832,44507780,795661,797399,19087394,705103,711584,718122,42904177,734354,19006586,715458,742267,19020002,744798,740910,734275,751347)
...
-- End Condition Occurrence Criteria
  ...
  WHERE DATEADD(day,CAST(3650 as bigint),OP.OBSERVATION_PERIOD_START_DATE) <= E.START_DATE AND DATEADD(day,CAST(0 as bigint),E.START_DATE) <= OP.OBSERVATION_PERIOD_END_DATE
) P
WHERE P.ordinal = 1
-- End Primary Events
