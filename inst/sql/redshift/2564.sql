...
WHERE C.procedure_date >= TO_DATE(TO_CHAR(2000,'0000FM')||'-'||TO_CHAR(1,'00FM')||'-'||TO_CHAR(1,'00FM'), 'YYYY-MM-DD')
-- End Procedure Occurrence Criteria

  ) E
	JOIN @cdm_database_schema.observation_period OP on E.person_id = OP.person_id and E.start_date >=  OP.observation_period_start_date and E.start_date <= op.observation_period_end_date
  WHERE DATEADD(day,CAST(1825 as bigint),OP.OBSERVATION_PERIOD_START_DATE) <= E.START_DATE AND DATEADD(day,CAST(0 as bigint),E.START_DATE) <= OP.OBSERVATION_PERIOD_END_DATE
) P
WHERE P.ordinal = 1
-- End Primary Events
...
WHERE (EXTRACT(YEAR FROM E.start_date) - P.year_of_birth >= 40 and EXTRACT(YEAR FROM E.start_date) - P.year_of_birth <= 80)
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,CAST(0 as bigint),P.START_DATE) AND A.START_DATE <= DATEADD(day,CAST(90 as bigint),P.START_DATE) ) cc
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= DATEADD(day,CAST(-30 as bigint),P.START_DATE) ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
